    ...
    <div className={`fixed inset-0 z-[60] transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out`}>
      ...
      <nav className={`absolute right-0 top-0 h-full w-[80%] max-w-sm p-8 shadow-2xl flex flex-col
        ...
        <div className="flex flex-col gap-6 text-xl font-light">
          ...
          {/* Renderizamos los links dinámicamente según la sección */}
            ...
            <div key={link.label}>
              {link.href ? (
                ...
                <div className="flex flex-col gap-2">
                  <span className="text-sm uppercase opacity-50">{link.label}</span>
                  {link.subLinks?.map(sub => (
                    <Link key={sub.href} href={sub.href} onClick={onClose} className="pl-4 text-lg">
                      {sub.label}
                    </Link>
                  ))}
                </div>
          ...
          <div className="mt-auto flex gap-4">
              <a href={`https://instagram.com/${config.instagram}`} target="_blank"><Instagram size={24} color="#E1306C"/></a>
              <a href={`https://wa.me/${config.phone}`}><Phone size={24} color="#25D366" /></a>
          </div>
        </div>
